...
- [2026-06-16] 商品別売上ページの参照元を当初CF入金(cfInFee/Training/Event)にしたが「CFではなくPLを引用して」と指摘 → 月次財務PLは売上が合計`rev`のみで商品別内訳が無いのが原因。方針A: PLに商品別売上キー `revFee/revTraining/revEvent` を新設(makeEmptyYearStruct/REQUIRED_PL_KEYS/万円→千円PL_KEYSに追加)し、商品別ページの参照をd.cf→d.plへ変更。月次テーブルは Cells.attach('prod-pl-table',{read/write→DB[yr].pl, enqueueCell(yr,'fiscal_pl_budget'…)+localStorage即時}) で予算/実績とも直接編集可。売上合計(rev)とは別キーで保持し「商品計 vs PL売上合計(rev)」の突合行＋差異⚠を表示(自動合算はしない=手入力主導)。学び: 売上の「入金(CF)」と「計上(PL)」は別概念。商品別の粒度はPL側に無いことが多いので、PL基準を求められたら専用キーを新設し既存rev合計とは突合で整合させる(rev集計やMF逆算には新キーを混ぜない)。
- [2026-06-16] 商品別売上進捗ページを新規タブ `#sec-products`(window.PRODUCTS, IIFE)で追加。商品=会費(cfInFee+後方互換cfInMembership)/研修(cfInTraining)/イベント・その他(cfInEvent)。進捗は「年間目標(reportData.salesTargets に保存→saveDBでSupabase往復+localStorage即時)」と「月次予算(cf.budget)累積比」の両建て。DB[year].cfは読み取り専用で流用し既存KPI/CF/月次財務は無改変。タブ追加の定石: ①タブボタン(data-ent="neo solo" data-tab, onclick sw())②`<div class="sec" id="sec-xxx">`③sw()に描画フック④tabOrder配列に追加(DOMの.tab順と一致必須=index復元のため)⑤スピナー除去CSS `#sec-xxx input[type=number]`。Chartは _chart を destroy→再生成、色はCSS変数不可なのでgetComputedStyleで解決した値を渡す。年間目標は onchange(blur)で保存→render再描画(inputフォーカス喪失回避)。商品=PLの売上計上(revFee/revTraining/revEvent + カスタム revC<ts>)で保持し、reportData.productDefs(年度別)で追加/削除(window.PRODUCTS.add(year)/remove(key,year))。
- [2026-06-16] 売上を商品別に内訳化する要望を月次財務PL(renderPLView)にも展開(方針A=内訳行表示＋合計は突合)。売上合計(rev)直下に _prodDefs(基本3+reportData.productDefs)の編集可サブ行(data-cell-key=pl.budget.<key>で既存pl-table-wrap Cellsが汎用処理)＋「商品計」計算行を挿入。売上合計(rev)はMF/手入力のまま不変で商品計と突合(自動合算しない)。「＋売上項目を追加」ボタンは window.PRODUCTS.add(currentYear)、カスタム行ラベルに✕で remove(key,currentYear)。add/remove は商品別売上ページと月次財務PLの両方を refreshViews() で再描画。重要: renderPLView の差異符号判定が `row.key==='rev'` のみで売上サブ行(isRev)が原価扱いになるため `row.key==='rev'||row.isRev` に修正(粗利の差異色も正常化)。PRODUCTS.add/remove は yearOverride 引数で対象年度を明示(月次財務からは currentYear、商品別ページからは選択年度 yr())。
- [2026-06-16] 売上項目のリネーム機能 `window.PRODUCTS.rename(key, yearOverride)` を追加。カスタム項目は `reportData.productDefs[].label` を直接更新、基本3項目(revFee/revTraining/revEvent)はキー固定のまま `reportData.productLabels[key]` に上書きを保存(既定文言に戻したら該当上書きを delete)。ラベル解決は defs() が productLabels で BASE_DEFS を上書き、renderPLView も _plLabelOv で基本ラベルを上書き。UI は ✎ アイコンを①商品別売上の進捗カード②同ページPL月次表のラベルセル③月次財務PL売上サブ行 の全てに配置(カスタムは✕削除も併設)。改名はキー不変なので入力済みの予算/実績データは保持される。productLabels は reportData 配下なので saveDB(全DB[year]送信)でSupabaseに自動永続化。
- [2026-06-16] 商品別売上に月次目標管理を追加。`reportData.salesTargetsMonthly[key]=[12]` で月次目標を保持。月次表(prod-pl-table)に商品ごと「🎯目標」行(編集可)を予算/実績の上に追加し、商品ラベル＋✎✕操作はこの先頭行に集約。実績セルの色は「対目標(目標未設定月は対予算)」、合計列に対目標 累積達成率を表示。カードは年間目標を annualTargetOf()=月次合計(入力時)→年間スカラー→年間予算 の優先で解決し、月次目標があれば年間目標inputは読取専用(月次合計)表示＋「🎯月次目標比(累積)」バーを追加。重要: prod-pl の Cells.write は data-cell-key の汎用パス走査(DB[yr].reportData.salesTargetsMonthly[key])で目標も編集可だが、enqueueCell は parts[0]==='pl'||'cf' に限定(reportDataは存在しないシートへ積まず、scheduleAutoSave の全行saveDBで同期)。年間スカラー salesTargets は月次未入力時のフォールバックとして温存。
